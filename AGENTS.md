...
- `ingest` and `compact` are dry-run by default. `--apply` writes; `--commit` additionally creates one git commit in the memory dir after three pre-flight checks (see `git.py` and the README commit-integration section).
- Runtime dependency: `brigade-cli>=0.8.0`. `paths.py` imports `MEMORY_INDEX_MAX_LINES` from `brigade.budgets` as the canonical default for the MEMORY.md line threshold. If tempted to hardcode that default locally: do not; import it.
...
- Default `--memory-dir` and `--handoffs-dir` resolve to the operator's REAL live memory and handoffs dirs, derived from $HOME in `paths.py`. Running `ingest --apply` or `compact --apply` with default paths mutates real operator memory.
...
- All file mutations must go through `atomic_write_text` and the target-containment check in `safety.py` (resolved targets must stay inside the memory dir). New write paths that bypass these are bugs.
...
## Pushing
- `core.hooksPath` is `hooks/`, so `hooks/pre-push` runs a content-guard scan on every push. It expects a clone at `~/repos/content-guard` (override with `CONTENT_GUARD_DIR`) and blocks the push on violations or if content-guard is missing.
